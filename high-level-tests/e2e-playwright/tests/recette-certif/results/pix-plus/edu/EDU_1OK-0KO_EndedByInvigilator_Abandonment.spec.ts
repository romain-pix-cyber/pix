...
const testRef = 'EDU_1OK-0KO_EndedByInvigilator_Abandonment';
const snapshotPath = `recette-certif/${testRef}/${testRef}.json`;
const csvResultPath = `recette-certif/${testRef}/${testRef}_csvresult.json`;

test(
  `${testRef} - User takes a certification test for a Pix+ Edu subscription. One challenge only answered. Ended by invigilator for abandonment`,
  {
    tag: ['@snapshot'],
    annotation: [
      {
